...
function ENT:Find(s)
	...
	if (ValidEntity(e) and not table.HasValue(e.__rep_count,self)) then table.insert(e.__rep_count,self) end;
	return e;
end
...
function Help.Clean_Table(t)
	...
	for k,v in pairs(t) do
		if (not ValidEntity(v) or k > GetConVarNumber("replicator_limit_bunch")) then
			table.remove(t,k);
		end
	end
end
